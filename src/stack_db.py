    ...
    try:
        ...
        with pg2.connect(db) as conn:
            ...
            if len(where_values) > 0:
                crsr.execute(sql,  tuple(where_values))
...
def insert_test_data(db: str,
    ...
    sql = "INSERT INTO stack_db.test_data (uid, msg"
    ...
    try:
        with pg2.connect(db) as conn:
            ...
            conn.commit()
        records, error = select_test_data(db=db, where={'uid=': uid})
        if len(records) > 0:
            record = records[0]
    except Exception as e:
        error = f"{e}"
...
if __name__ == '__main__':
    ...
    if error is None:
        test_msg = {'KEY1': 'blob',
                    ...
                    'greeting': 'effoff'}
        uid = "hdhdh"
        record, error = insert_test_data(db=db, uid=uid, msg=test_msg)
        print(record)
